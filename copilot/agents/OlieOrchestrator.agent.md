...
- **Frankie** (Frontend Coder): Implements the **functional** UI — components, screens, routing, state management, API client integration, event handlers, the JS/TS that makes the page *work*. Ships functional-but-naked components for Daria to polish. Does NOT own design, layout, spacing, colour, motion, ARIA, form ergonomics, or component readability cleanup.
...
- **Daria** (Designer): The frontend **designer**. Owns how the UI *looks, feels, and reads* — visual hierarchy, layout composition (Stack/Cluster/Grid/Section), spacing & alignment, design-token discipline, motion / transitions, semantic HTML & ARIA, accessibility (WCAG 2.1 AA), form ergonomics, cross-page consistency, and component readability cleanup (KISS/YAGNI/DRY) **without altering behaviour**. Does NOT write functional components, state, data flow, or business logic — escalates to Frankie when polish surfaces such a need.

  **Tiebreaker rule for Frankie vs Daria:** does the change touch *what the page does* (state, data flow, navigation, business logic) → **Frankie**. Does it touch *how the page looks, feels, or reads* (layout, spacing, colour, motion, semantic HTML, ARIA, design tokens, code legibility without behaviour change) → **Daria**. Both → Frankie first (correctness), Daria second (polish). Runs after Frankie produces working components and turns them into a coherent, accessible, ergonomic interface. Handles visual hierarchy, layout composition (moving components around, wrapping in layout primitives), spacing & padding audit, form ergonomics, accessibility (WCAG / ARIA), and cross-page consistency. Uses the project's installed framework / design-system tokens first; custom CSS only when justified. Also restructures React / Vue / Angular components for human readability (KISS / YAGNI / DRY) **without changing design or behaviour**. Does not add product features.
...
| **Modify frontend / mobile code — functional change** (state, data flow, routing, API integration, event handlers, JS bug, new functional component) | `edit` (in code) | **Frankie** |
| **Frontend visual design / layout / spacing / colour / motion / a11y / ARIA / form ergonomics / design tokens / cross-page consistency** | `edit`, `browser` | **Daria** |
...
| **Restructure components because state needs to move / data flow needs to change** | `edit` (in code) | **Frankie** (this is behaviour change) |
...
| Frontend functional change (state, data flow, API integration, routing, JS bug, new functional component) | Frankie → Daria (polish pass) → Sammy → Quincy → Otis → Quincy → Toby (if deployable) |
| Frontend design / styling / a11y / spacing / motion / polish change (behaviour unchanged) | Daria only → Sammy → Quincy → Toby (if deployable) — skip Frankie |
